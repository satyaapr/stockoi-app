...
type ReceivingAutomationWorkspaceProps = {
...
export function ReceivingAutomationWorkspace({ samples, acceptedDocumentTypes, acceptedBatchTypes }: ReceivingAutomationWorkspaceProps) {
  ...
  async function handleFileChange(event: ChangeEvent<HTMLInputElement>) {
    ...
    setRawText([
      ...
      'This demo accepts PDF, JPG, JPEG, or PNG files for Receiving Automation.',
      'Click “Extract Fields” to continue using the structured output template below.',
    ].join('\n'));
    setMessage(`Single document upload detected. ${file.name} is ready for Receiving Automation extraction.`);
    if (selectedSample) {
        ...
        <div className="grid gap-4 md:grid-cols-[220px_minmax(0,1fr)]">
          <div className="space-y-3">
            ...
            <div className="rounded-2xl border border-dashed border-slate-300 bg-slate-50 p-4 text-sm text-slate-500">
              ...
              <div className="mt-2 leading-6">Receiving Automation turns receiving documents into structured inventory updates before validation rules are applied. CSV/XLS/XLSX files are routed as batch uploads.</div>
            </div>
          </div>
